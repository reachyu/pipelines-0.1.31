...
class NewRun extends Page<{}, NewRunState> {
    ...
    return (
      <div className={classes(commonCss.page, padding(20, 'lr'))}>
        <div className={commonCss.scrollContainer}>
          ...
          <div className={commonCss.header}>实例详情</div>
          ...
          {!useWorkflowFromRun && (
            <Input value={pipelineName} required={true} label='工作流' disabled={true}
              variant='outlined'
              InputProps={{
          ...
          <div className={commonCss.header}>实例类型</div>
          {isClone && (
            <span>{isRecurringRun ? 'One-off' : 'Recurring'}</span>
          )}
          {!isClone && (
            <React.Fragment>
              <FormControlLabel id='oneOffToggle' label='实时' control={<Radio color='primary' />}
                onChange={() => this._updateRecurringRunState(false)}
                checked={!isRecurringRun} />
              <FormControlLabel id='recurringToggle' label='定时' control={<Radio color='primary' />}
          ...
          <div className={classes(commonCss.flex, padding(20, 'tb'))}>
            <BusyButton id='startNewRunBtn' disabled={!!errorMessage}
              ...
              className={commonCss.buttonAction} title='创建'
            ...
            <Button id='exitNewRunPageBtn' onClick={() => {
              ...
              {isFirstRunInExperiment ? 'Skip this step' : '取消'}
            </Button>
  ...
  public async load(): Promise<void> {
    ...
    let experimentName = '';
    const breadcrumbs = [{ displayName: '工程', href: RoutePage.EXPERIMENTS }];
    if (experimentId) {
      try {
    ...
    const titleVerb = originalRunId ? '复制' : '创建';
    const pageTitle = isRecurringRun ? `${titleVerb}定时任务` : `${titleVerb}实例`;
