...
class RunList extends React.PureComponent<RunListProps, RunListState> {
  ...
  public render(): JSX.Element {
    ...
    const columns: Column[] = [
      ...
      { label: '时间', flex: 0.5 },
      { customRenderer: this._pipelineCustomRenderer, label: '工作流', flex: 1 },
      { customRenderer: this._recurringRunCustomRenderer, label: '定时任务', flex: 0.5 },
      { label: '开始时间', flex: 1, sortKey: RunSortKeys.CREATED_AT },
    ];

    if (!this.props.hideExperimentColumn) {
      columns.splice(3, 0, { customRenderer: this._experimentCustomRenderer, label: '工程', flex: 1 });
    }
    ...
    return (<div>
      <CustomTable columns={columns} rows={rows} selectedIds={this.props.selectedIds}
        initialSortColumn={RunSortKeys.CREATED_AT} ref={this._tableRef} filterLabel='搜索'
        ...
        emptyMessage={
          `${this.props.experimentIdMask ? '工程里' : ''}没有` +
          `${this.props.storageState === RunStorageState.ARCHIVED ? '已归档的' : '有效的'}` +
          `实例数据.`
        }
      />
    </div>);
  ...
  protected async _loadRuns(request: ListRequest): Promise<string> {
    ...
    if (Array.isArray(this.props.runIdListMask)) {
      ...
      try {
        ...
        this.props.onError('错误: 获取实例失败.', error);
