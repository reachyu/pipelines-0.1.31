...
type ClientManager struct {
	...
	appManageStore         storage.AppManageStoreInterface
	myAppManageStore       storage.MyAppManageStoreInterface
}

func (c *ClientManager) MyAppManageStore() storage.MyAppManageStoreInterface {
	return c.myAppManageStore
}

func (c *ClientManager) AppManageStore() storage.AppManageStoreInterface {
	return c.appManageStore
}
...
func (c *ClientManager) init() {
	...
	c.appManageStore = storage.NewAppManageStore(db, c.time, c.uuid)
	c.myAppManageStore = storage.NewMyAppManageStore(db, c.time, c.uuid)
...
func initDBClient(initConnectionTimeout time.Duration) *storage.DB {
	...
	// 一个坑，不设置这个参数，gorm会把表名转义后加个s
	// 比如model\app_manage.go，如果没有下面的代码，那么表名称就是app_manages
	// 全局设置表名不可以为复数形式
	/**
	      禁用表名复数>
...
func initMysql(driverName string, initConnectionTimeout time.Duration) string {
	...
	glog.Infof("=====================mysqlServiceHost=====================: %s", common.GetStringConfigWithDefault(mysqlServiceHost, "mysql"))
	glog.Infof("=====================mysqlPassword=====================: %s", common.GetStringConfigWithDefault(mysqlPassword, "mysql"))

	var db *sql.DB
	var err error
	var operation = func() error {
		glog.Infof("=====================dataSourceName=====================: %s", mysqlConfig.FormatDSN())
