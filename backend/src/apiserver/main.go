...
func startHttpProxy(resourceManager *resource.ResourceManager) {
	...
	//新加接口链接
	appManageServer := server.NewAppManageServer(resourceManager)
	topMux.HandleFunc("/apis/v1beta1/app/create", appManageServer.CreateApp)
	topMux.HandleFunc("/apis/v1beta1/app/get", appManageServer.GetApp)
	topMux.HandleFunc("/apis/v1beta1/app/test", func(w http.ResponseWriter, r *http.Request) {
		io.WriteString(w, `{"commit_sha":"test app "}`)
	})

	//增加应用管理接口
	myAppManageServer := server.NewMyAppManageServer(resourceManager)
	topMux.HandleFunc("/apis/v1beta1/myapp/create", myAppManageServer.CreateMyApp)
	topMux.HandleFunc("/apis/v1beta1/myapp/get", myAppManageServer.GetMyApp)
